/*
AUTORES: Rivas Solis Carlos Eduardo, Castelan Ramos Carlos, Juarez García Hugo y Torres Marco
Fecha: 20/05/22
Descripción: Query para la creación general de la base de datos (tablas y constraints)
*/

--Creación de la base de datos
create database PYME0
go
...
ALTER TABLE venta.online 
	DROP CONSTRAINT FK_CLIENTE_ONLINE
GO

alter table venta.online
	drop column cancelacion, fechaCancelacion, multa, idCliente
go

alter table persona.suscripcion
	alter column
	notificacion bit null
GO

alter table persona.suscripcion 
	add constraint DF_NOTIFICACION default 0 for notificacion
go

alter table persona.vendedor
...
alter table catalogo.cesta
	add idCliente int not null constraint FK_CLIENTE_CESTA foreign key 
		references persona.cliente(idCliente) 
go

alter table catalogo.cesta
	alter column cantidadProducto int null 
go

alter table catalogo.cesta
	add constraint DF_CANTIDAD_PRODUCTO default 0 for cantidadProducto
go
----------------------------22/05/2022----------------------------
ALTER TABLE catalogo.lista DROP CONSTRAINT CK_Permanencia
GO

alter table catalogo.lista add
	constraint CK_Permanencia check(Day(getdate()-fechaAgrega)<18)
go

--Alterando tabla promocion.oferta
ALTER TABLE promocion.oferta DROP CONSTRAINT CK_Lapso
GO

alter table promocion.oferta add
	constraint CK_Lapso check(lapso < 43)
	go
...
ADD cancelacion bit null default 0
go